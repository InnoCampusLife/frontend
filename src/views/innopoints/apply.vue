						...
						li.list-group-item.py-1(v-for='(u, index) of current.users')
							...
							.form-group.row.flex-items-sm-middle
								...
								.col-sm
									select(
										class="form-control",
										:disabled="!(isCategorySelected && isUsernameCorrect)",
										:id="'activity_' + index",
										class="activity",
			...
			username_changed(e) {
				this.user.account.getBio(
					...
					(result) => {
						...
						this.current.users[e.target.dataset.index].user_id = result.id
					},
					(error) => {
						console.log("Incorrect Username");
						this.isUsernameCorrect = false
						this.current.users[e.target.dataset.index].activity_id = ''
						this.current.users[e.target.dataset.index].amount = 1
						// TODO
					}
				)
